...
bool User::hasPassword(const string &password){
...
std::list<long>* User::getEventIDs(){
...
bool User::canWrite(const long eventID){
...
bool User::writeToFile(const string &filename){
  ofstream file;
  file.open(filename);
  if(file.fail())
    return false;
